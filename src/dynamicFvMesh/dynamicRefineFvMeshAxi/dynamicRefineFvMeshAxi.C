...
Foam::autoPtr<Foam::mapPolyMesh> Foam::dynamicRefineFvMeshAxi::refine
(
...
// (an approximate) flux
Foam::autoPtr<Foam::mapPolyMesh> Foam::dynamicRefineFvMeshAxi::unrefine
(
...
Foam::labelList Foam::dynamicRefineFvMeshAxi::selectUnrefineEdges
(
...
template <class T>
void Foam::dynamicRefineFvMeshAxi::mapNewInternalFaces
(
...
)
{
    ...
    forAllIter(typename HashTable<GeoField*>, sFlds, iter)
    {
        ...
        {
Info << "dynamicRefineFvMeshAxi::mapNewInternalFaces(): iter.key()" <<iter.key() << endl;
            ...
            for (label facei = 0; facei < nInternalFaces(); facei++)
            {
                ...
                if (oldFacei == -1)
                {
                    ...
                    if(counter > 0)
                    {
                        ...
                        {
        FatalErrorIn("dynamicRefineFvMeshAxi::mapNewInternalFaces()")
                                << "mapping implementation only valid for"
                                << " scalar and vector fields! \n Field "
                                << sFld.name() << " is of type: "
                                << GeometricField<T, fvsPatchField, surfaceMesh>::typeName << abort(FatalError);
                        }
                    }
...
) const
{
    ...
    const_cast<hexRefAxi&>(meshCutter_).setInstance(time().timeName());
