...
Foam::autoPtr<Foam::mapPolyMesh> Foam::dynamicRefineFvMesh2D::refine
(
...
// (an approximate) flux
Foam::autoPtr<Foam::mapPolyMesh> Foam::dynamicRefineFvMesh2D::unrefine
(
...
template <class T>
void Foam::dynamicRefineFvMesh2D::mapNewInternalFaces
(
...
)
{
    ...
    forAllIter(typename HashTable<GeoField*>, sFlds, iter)
    {
        ...
        {
Info << "dynamicRefineFvMesh::mapNewInternalFaces(): iter.key()" <<iter.key() << endl;
            ...
            for (label facei = 0; facei < nInternalFaces(); facei++)
            {
                ...
                if (oldFacei == -1)
                {
                    ...
                    if(counter > 0)
                    {
                        ...
                        {
        FatalErrorIn("dynamicRefineFvMesh2D::mapNewInternalFaces()")
