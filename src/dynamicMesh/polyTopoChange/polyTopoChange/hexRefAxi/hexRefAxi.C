...
namespace Foam
{
    defineTypeNameAndDebug(hexRefAxi, 0);
...
)
{
    ...
    forAll(elems, i)
    {
        ...
        if (newI >= len)
        {
            FatalErrorIn("hexRefAxi::reorder(..)") << abort(FatalError);
        }
...
void Foam::hexRefAxi::getFaceInfo
(
    const label faceI,
    label& patchID,
    label& zoneID,
...
Foam::label Foam::hexRefAxi::addFace
(
...
// for the face)
Foam::label Foam::hexRefAxi::addInternalFace
(
    polyTopoChange& meshMod,
...
Foam::scalar Foam::hexRefAxi::getLevel0EdgeLength() const
{
    if (cellLevel_.size() != mesh_.nCells())
    {
        FatalErrorIn
        (
            "hexRefAxi::getLevel0EdgeLength() const"
    ...
    if (debug)
    {
        Pout<< "hexRefAxi::getLevel0EdgeLength() :"
    ...
    if (level0Size == -1)
    {
        FatalErrorIn("hexRefAxi::getLevel0EdgeLength()")
...
Foam::label Foam::hexRefAxi::getAnchorCell
(
...
) const
{
    if (cellAnchorPoints[cellI].size())
    {
        ...
        FatalErrorIn("hexRefAxi::getAnchorCell(..)")
...
void Foam::hexRefAxi::getFaceNeighbours
(
    const labelListList& cellAnchorPoints,
    const labelListList& cellAddedCells,
    const label faceI,
...
Foam::label Foam::hexRefAxi::findMinLevel(const labelList& f) const
{
...
Foam::label Foam::hexRefAxi::findMaxLevel(const labelList& f) const
{
...
Foam::label Foam::hexRefAxi::countAnchors
(
    const labelList& f,
...
void Foam::hexRefAxi::dumpCell(const label cellI) const
{
    OFstream str(mesh_.time().path()/"cell_" + Foam::name(cellI) + ".obj");
    Pout<< "hexRefAxi : Dumping cell as obj to " << str.name() << endl;
...
Foam::label Foam::hexRefAxi::findLevel
(
    const face& f,
    const label startFp,
...
) const
{
    ...
    forAll(f, i)
    {
        ...
        {
            FatalErrorIn("hexRefAxi::findLevel(..)")
                << "face:" << f
...
Foam::label Foam::hexRefAxi::getAnchorLevel(const label faceI) const
{
    const face& f = mesh_.faces()[faceI];
...
void Foam::hexRefAxi::checkInternalOrientation
(
    polyTopoChange& meshMod,
    const label cellI,
    const label faceI,
...
void Foam::hexRefAxi::checkBoundaryOrientation
(
    polyTopoChange& meshMod,
    const label cellI,
    const label faceI,
...
// splitPoint.
void Foam::hexRefAxi::insertEdgeSplit
(
...
Foam::label Foam::hexRefAxi::storeMidPointInfo
(
    const labelListList& cellAnchorPoints,
    const labelListList& cellAddedCells,
    ...
    if (changed && haveTwoAnchors)
    {
        ...
        forAll(cFaces, i)
        {
            ...
            if (faceMidPoint[faceJ] != faceMidPointI
                ...
                && faceMidPoint[faceJ] != 1234567890) // could be replace by faceJ == type "wedge"
            {
        ...
        // Should I check here that anchorPointI does not appear twice in the anchorPoints?
...
void Foam::hexRefAxi::createInternalFaces
(
    const labelListList& cellAnchorPoints,
    const labelListList& cellAddedCells,
    const labelList& cellMidPoint,
...
void Foam::hexRefAxi::walkFaceToMid
(
    const labelList& edgeMidPoint,
    const label cLevel,
    const label faceI,
...
void Foam::hexRefAxi::walkFaceFromMid
(
    const labelList& edgeMidPoint,
    const label cLevel,
    const label faceI,
...
Foam::label Foam::hexRefAxi::faceConsistentRefinement
(
    const bool maxSet,
...
void Foam::hexRefAxi::checkWantedRefinementLevels
(
...
) const
{
    ...
    for (label faceI = 0; faceI < mesh_.nInternalFaces(); faceI++)
    {
        ...
        if (mag(ownLevel-neiLevel) > 1)
        {
            ...
            FatalErrorIn
            (
                "hexRefAxi::checkWantedRefinementLevels(const labelList&)"
            )   << "cell:" << own
...
void Foam::hexRefAxi::setInstance(const fileName& inst)
{
    if (debug)
    {
        Pout<< "hexRefAxi::setInstance(const fileName& inst) : "
...
Foam::hexRefAxi::hexRefAxi(const polyMesh& mesh, const bool readHistory)
:
    mesh_(mesh),
    cellLevel_
    (
    ...
    savedCellLevel_(0)
{
    ...
    if (history_.active() && history_.visibleCells().size() != mesh_.nCells())
    {
        FatalErrorIn
        (
            "hexRefAxi::hexRefAxi(const polyMesh&)"
    ...
    savedCellLevel_(0)
{
    if (history_.active() && history_.visibleCells().size() != mesh_.nCells())
    {
        FatalErrorIn
        (
            "hexRefAxi::hexRefAxi(const polyMesh&, const labelList&"
...
Foam::labelList Foam::hexRefAxi::cellPoints(const label cellI) const
{
...
Foam::labelList Foam::hexRefAxi::consistentRefinement
(
    const labelList& cellsToRefine,
    const bool maxSet
) const
{
    ...
    while (true)
    {
        ...
        if (debug)
        {
            Pout<< "hexRefAxi::consistentRefinement : Changed " << nChanged
...
Foam::labelList Foam::hexRefAxi::consistentSlowRefinement
(
...
)
{
    ...
    if (maxFaceDiff <= 0)
    {
        FatalErrorIn
        (
            "hexRefAxi::consistentSlowRefinement"
    ...
    while (true)
    {
        if (debug)
        {
            Pout<< "hexRefAxi::consistentSlowRefinement : Seeded "
    ...
    if (debug)
    {
        Pout<< "hexRefAxi::consistentSlowRefinement : From "
...
Foam::labelList Foam::hexRefAxi::consistentSlowRefinement2
(
    const label maxFaceDiff,
    const labelList& cellsToRefine,
    const labelList& facesToCheck
) const
{
    ...
    if (maxFaceDiff <= 0)
    {
        FatalErrorIn
        (
            "hexRefAxi::consistentSlowRefinement2"
            "(const label, const labelList&, const labelList&)"
        )   << "Illegal maxFaceDiff " << maxFaceDiff << nl
            << "Value should be >= 1" << exit(FatalError);
    }
    ...
    while (true)
    {
        ...
        if (debug)
        {
            Pout<< "hexRefAxi::consistentSlowRefinement2 : Changed " << nChanged
                << " refinement levels due to 2:1 conflicts."
                << endl;
        }
...
//        if (refineCell.get(cellI))
    ...
    if (debug)
    {
        Pout<< "hexRefAxi::consistentSlowRefinement2 : From "
            << cellsToRefine.size() << " to " << newCellsToRefine.size()
            << " cells to refine." << endl;
...

        {
            ...
            Pout<< "hexRefAxi::consistentSlowRefinement2 : writing "
...
Foam::labelListList Foam::hexRefAxi::setRefinement
(
...
//        if (isA<wedgePolyPatch>(mesh_.boundaryMesh()[patchID])
    ...
    if (debug)
    {
        ...
        Pout<< "hexRefAxi::setRefinement : Dumping " << splitCells.size()
    ...
    if (debug)
    {
        ...
        Pout<< "hexRefAxi::setRefinement : Dumping " << splitFaces.size()
                ...
                forAll(f, fp)
                {
                    ...
                    )
                    {
                        if (nAnchorPoints[cellI] == 8)
                        {
                            ...
                            FatalErrorIn
                                (
                                 "hexRefAxi::setRefinement(const labelList&"
                                 ", polyTopoChange&)"
                                ) << "cell " << cellI
    ...
    if (debug)
    {
        Pout<< "hexRefAxi::setRefinement : Splitting faces" << endl;
    }
    ...
    if (debug)
    {
        ...
        {
            FatalErrorIn("hexRefAxi::setRefinement(..)")
...
void Foam::hexRefAxi::updateMesh(const mapPolyMesh& map)
{
...
// sure the pointLevel_ and cellLevel_ are the size of the new mesh so we
// only need to account for reordering.
void Foam::hexRefAxi::updateMesh
(
...
)
{
...

    {
        ...
        {
            // Assume it is after hexRefAxi that this routine is called.
        ...
        forAllConstIter(Map<label>, cellsToRestore, iter)
        {
            ...
            {
                FatalErrorIn("hexRefAxi::updateMesh(const mapPolyMesh&)")
        ...
        //{
        //    WarningIn("hexRefAxi::updateMesh(const mapPolyMesh&)")
        //        << "Problem : "
...
)
{
    ...
    if (history_.active())
    {
        WarningIn
        (
            "hexRefAxi::subset(const labelList&, const labelList&"
            ", const labelList&)"
...
void Foam::hexRefAxi::distribute(const mapDistributePolyMesh& map)
{
    if (debug)
    {
        Pout<< "hexRefAxi::distribute :"
            << " Updating various lists"
            << endl;
    }
...
void Foam::hexRefAxi::checkMesh() const
{
    const scalar smallDim = 1e-6 * mesh_.bounds().mag();

    if (debug)
    {
        Pout<< "hexRefAxi::checkMesh : Using matching tolerance smallDim:"
            << smallDim << endl;
    }
        ...
        forAll(patches, patchI)
        {
            ...
            if (pp.coupled())
            {
                ...
                forAll(pp, i)
                {
                    ...
                    {
                        dumpCell(own);
                        FatalErrorIn("hexRefAxi::checkMesh()")
    ...
    if (debug)
    {
        Pout<< "hexRefAxi::checkMesh : Returning" << endl;
    }
}


void Foam::hexRefAxi::checkRefinementLevels
(
    const label maxPointDiff,
    const labelList& pointsToCheck
) const
{
    if (debug)
    {
        Pout<< "hexRefAxi::checkRefinementLevels :"
    ...
    {
        FatalErrorIn("hexRefAxi::checkRefinementLevels(const label)")
...

    {
        ...
        for (label faceI = 0; faceI < mesh_.nInternalFaces(); faceI++)
        {
            ...
            if (mag(cellLevel_[own] - cellLevel_[nei]) > 1)
            {
                ...
                FatalErrorIn
                (
                    "hexRefAxi::checkRefinementLevels(const label)"
...
Foam::labelList Foam::hexRefAxi::getSplitEdges() const
{
    if (!history_.active())
    {
        FatalErrorIn("hexRefAxi::getSplitPoints()")<< "Only call if constructed with history capability"
            << abort(FatalError);
    }
    // Master cell
    // -1 undetermined
...
//void Foam::hexRefAxi::markCellClusters
//(
//    const label maxLevel,
//    labelList& cluster
//) const
...
Foam::labelList Foam::hexRefAxi::consistentUnrefinement
(
...
) const
{
    if (debug)
    {
        Pout<< "hexRefAxi::consistentUnrefinement :"
    ...
    if (maxSet)
    {
        FatalErrorIn
        (
            "hexRefAxi::consistentUnrefinement(const labelList&, const bool"
        ...
        for (label faceI = 0; faceI < mesh_.nInternalFaces(); faceI++)
        {
            ...
            if (ownLevel < (neiLevel-1))
            {
                ...
                else
                {
                    ...
                    {
                        FatalErrorIn("hexRefAxi::consistentUnrefinement(..)")
...
void Foam::hexRefAxi::setUnrefinement
(
...
)
{
    if (!history_.active())
    {
        FatalErrorIn
        (
            "hexRefAxi::setUnrefinement(const labelList&, polyTopoChange&)"
    ...
    forAll(splitEdgeLabels, i)
    {
        ...
        //if (debug)
        {
            ...
            forAll(eCells, j)
            {
                ...
                if (region == -1)
                {
                    FatalErrorIn("hexRefAxi::setUnrefinement(..)")
...
bool Foam::hexRefAxi::write() const
{
